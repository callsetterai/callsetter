 ...
 * - Fixed navbar with logo (/logo.png)
 * - Hero + animated visuals
 * - Why/How/Services/ROI/FAQ + Final CTA
 * - Modal form (name, email, phone)
 */

const LOGO_SRC = "/logo.png"; // upload your real logo as public/logo.png
...
export default function Page() {
  ...
  // quick dev tests for ROI calc
  useEffect(() => {
    const calc = (L: number, R: number, V: number) => Math.round(L * (R / 100) * V * 0.2);
    console.assert(calc(300, 10, 1500) === 9000, "Test#1 300/10%/1500 -> 9000");
    console.assert(calc(100, 20, 1000) === 4000, "Test#2 100/20%/1000 -> 4000");
    console.assert(calc(0, 10, 1500) === 0, "Test#3 zero leads -> 0");
    console.assert(calc(500, 0, 2000) === 0, "Test#4 zero close rate -> 0");
  }, []);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // TODO: Paste your voice-agent webhook URL here and uncomment:
  ...
  return (
    <div className="bg-[#0a0a0a] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <section id="top" className="relative min-h-screen flex flex-col justify-center items-center text-center px-6 overflow-hidden pt-32">
        ...
        />
        {/* parallax “phone” glows */}
        <motion.div style={{ y: phoneYOffset, opacity: glowOpacity }} className="pointer-events-none absolute w-[320px] h-[640px] md:w-[420px] md:h-[780px] bg-gradient-to-b from-cyan-400/20 to-transparent rounded-[2.2rem] border border-cyan-400/30 backdrop-blur-xl rotate-6 right-[8%] top-[18%]" />
        <motion.div style={{ y: phoneYOffset, opacity: glowOpacity }} className="pointer-events-none absolute w-[260px] h-[560px] bg-gradient-to-b from-blue-400/20 to-transparent rounded-[2rem] border border-blue-400/30 backdrop-blur-xl -rotate-6 left-[8%] top-[24%]" />
        ...
        <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.15 }} className="flex gap-4 mt-8">
          <button onClick={() => setShowForm(true)} className="bg-white text-black px-8 py-3 font-semibold rounded-full hover:bg-gray-200 transition-all flex items-center gap-2">
      ...
      <section id="why" className="py-20 text-center px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 max-w-6xl mx-auto">
          {[
            ...
            <motion.div key={i} initial={{ y: 10, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: i * 0.05 }} viewport={{ once: true }} className="flex flex-col items-center bg-[#111] p-6 rounded-xl border border-gray-800">
      ...
      <section id="how" className="py-24 px-6 bg-[#0d0d0d]">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto text-center">
          {[
            ...
            <motion.div key={i} initial={{ y: 10, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="p-6 bg-[#1a1a1a] rounded-2xl border border-gray-800">
              {s.icon}
        ...
        <div className="max-w-5xl mx-auto mt-24">
          ...
          <div className="grid md:grid-cols-2 gap-4 text-gray-300 text-sm">
            {[
              ...
              "Voice customization incl. tone, pace, language",
      ...
      <section id="services" className="py-24 px-6 bg-black">
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto text-gray-300">
          {[
            ...
            <motion.div key={i} initial={{ y: 8, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: i * 0.06 }} className="p-4 bg-[#151515] rounded-xl border border-gray-800 flex items-center gap-3">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-8 bg-[#111] p-8 rounded-2xl border border-gray-800">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex flex-col justify-center items-center text-center">
            ...
            <div className="flex gap-4">
              <button onClick={() => setShowForm(true)} className="bg-white text-black px-6 py-3 rounded-full font-semibold hover:bg-gray-200">
                <PhoneCall className="w-4 h-4 inline-block mr-2" /> Test The AI Setter Now!
              </button>
              <button onClick={() => setShowForm(true)} className="border border-gray-600 px-6 py-3 rounded-full hover:bg-gray-800">Book a Demo</button>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="cta" className="py-24 text-center bg-gradient-to-b from-[#0a0a0a] to-black relative overflow-hidden">
        ...
        <motion.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.98 }} onClick={() => setShowForm(true)} className="bg-white text-black px-10 py-4 font-bold rounded-full text-lg hover:bg-gray-200 transition-all relative z-10 flex items-center gap-2 mx-auto">
      ...
      {showForm && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <motion.div initial={{ y: 20, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 22 }} className="relative z-10 w-full max-w-md rounded-2xl border border-white/10 bg-[#0f0f10] p-6 shadow-2xl" role="dialog" aria-modal="true" aria-label="Test the AI Setter">
